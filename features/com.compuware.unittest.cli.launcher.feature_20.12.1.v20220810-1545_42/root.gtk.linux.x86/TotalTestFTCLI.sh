...
if [ -z "$TTT_DB_DRIVERS_PATH" ]; then
  export "TTT_DB_DRIVERS_PATH="$(dirname "$0")"/dbDrivers"
fi

LOGBACK_SUFFIX=workspace/TTTlogback.xml
APPLICATION=com.compuware.xatester.cli.FunctionalTestCLI

# Call the master script
bash "$(dirname "$0")"/TopazWorkbenchCLI.sh ${LOGBACK_SUFFIX} ${APPLICATION} "$@"